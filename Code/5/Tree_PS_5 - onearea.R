...
bins <- seq(30, 150, 10)
Nsplit <- 1 # the number of splits (the number of cells - 1)
...
# ggsave(file=paste0(save_dir,"Trees_5_onearea.png"), width = 10, height = 8)

# extract catch flag derived from the regression tree package
Cell <- LF_Tree$LF$Flag1[which(LF_Tree$LF$dummy == TRUE)]
Cell[which(Cell>0)] <- 1
...
# write.csv(Catch_Fishery,file=paste0(save_dir,"PS_Catch_5_onearea.csv"),row.names = FALSE)
...
write.csv(N,file=paste0(save_dir,"5_N_onearea.csv"),row.names = FALSE)
