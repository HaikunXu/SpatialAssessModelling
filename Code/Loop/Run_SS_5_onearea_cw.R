...
for (re in c(54:84,86:100)) {
  dir <- paste0("C:/Users/hkxu/OneDrive - IATTC/IATTC/2021/Spatial-SA/Model/Loop/test_5_onearea_cw/",re,"/")
  setwd(dir)
  ...
  tryCatch({
    ...
    File[Line-4] <- "2"
    File[Line+1] <- paste0("6 8 1 ",toString(Francis_size), " 1")
  ...
  if(ss[length(ss)]!="Hessian does not appear to be positive definite") {
    Flag <- 1
    save(Flag,file="Flag.RData")
    print("Converged!!!")
  }
