...
# load PS catch data
load(paste0(save_dir,"PS_Catch_25.RData"))
Catch <- Catch_DF %>%
  mutate(year=ceiling(Year/4),quarter=(Year-1)%%4 +1) %>%
  rename(lat=Lat,lon=Lon) %>%
...
write.csv(Catch_Fishery,file=paste0(save_dir,"PS_Catch_25.csv"),row.names = FALSE)
...
write.csv(N,file=paste0(save_dir,"PS_25_N.csv"),row.names = FALSE)
