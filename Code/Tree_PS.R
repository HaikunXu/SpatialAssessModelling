...
LF <- LF_DF[,c(3:7)] %>%
  mutate(Length2=cut(Length,breaks = c(0,seq(40, 120, 10),220),right = F,labels = seq(30, 120, 10))) %>%
  mutate(year=ceiling(Year/4),quarter=(Year-1)%%4 +1) %>%
  group_by(year,quarter,Lat,Lon) %>%
  mutate(tot=sum(LF)) %>%
  filter(tot>0) %>% # remove all-0 rows  
...
lcol <- 14 # the last column with LF_Tree info
bins <- seq(30, 120, 10)
...
select <- c(1,10) # as.numeric(LF_Loop$Imp_DF_sorted[1,1:Nsplit])
LF_Tree <- run_regression_tree(LF,fcol,lcol,bins,Nsplit,save_dir,manual=TRUE,select=select, quarter = FALSE)
...
LF_Tree <- run_regression_tree(LF_new,fcol,lcol,bins,Nsplit,save_dir,manual=TRUE,select=select,include_dummy = TRUE, quarter = FALSE)
